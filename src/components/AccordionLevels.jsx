...
const AccordionLevels = () => {
  ...
  return (
    <>  
        {
            levels.map((level, id) => (
                <Accordion sx={{width: '100%', marginBottom: '50px', borderRadius: '20px'}} expanded={expanded === `panel${id +1}`} onChange={handleChange(`panel${id+1}`)}>
                    <AccordionSummary
                    ...
                    >
                        <div className="container">
                            <img className='img-acordeon' src={level.img} alt="nivel 1" />
                            <p className="text big-size color">{level.title}</p>
                        </div>
                    </AccordionSummary>
                    <AccordionDetails
                        sx={{
